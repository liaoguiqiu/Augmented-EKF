% LIAO GUI QIU ALgorithm

 addpath  my_data_process 
 addpath  kf_fuse 
%  my_dada_fuse_ini_paramerterr;
my_ini_2;
% kf_fuse_init;
lag_time=1;  %10ms 的间隔，200ms延时
...
ekf = [];
ekf.sigmas.mag3D_unitVector_meas = 10*sqrt(uavSensors.sigmas.mag3D_unitVector_noise^2+(5*uavSensors.sigmas.mag3D_unitVector_bias)^2);
ekf.sigmas.pos_meas_m            = 1*sqrt(uavSensors.sigmas.GPSpos_noise_m^2        +(5*uavSensors.sigmas.GPSpos_bias_m)^2        );
ekf.sigmas.vel_meas_mps          = 1*sqrt(uavSensors.sigmas.GPSvel_noise_mps^2      +(5*uavSensors.sigmas.GPSvel_bias_mps)^2      );
%  ekf.sigmas.pos_meas_m            = 1;
%  ekf.sigmas.vel_meas_mps          =0.05;
...
ekf.sigmas.pos_process_noise_m = 0.00000000001;           % Position process noise, m
ekf.sigmas.vel_process_noise_mps = 0.00001;         % Velocity process noise, m/s
ekf.sigmas.gyroBias_process_noise_rps = 1e-4; % Gyro bias process noise, rad/s
ekf.sigmas.accelBias_process_noise_mps2=7e-3; % Accel bias process noise, m/s^2
ekf.sigmas.timeerr_process_noise  = 0.0003;
...
%     ].^2);
ekf.P = diag([...
        [1 1 1 1]*0.1 ...        % init quaternion (NED-to-body) uncertainty
        [1 1 1]*100 ...         % init North-East-Alt position uncertainties, m
        [1 1 1]*100 ...       % init NED velocity uncertainties, m/s
        [1 1 1]*0.0001 ...        % init XYZ gyro bias uncertainties, rad/s
        [1 1 1]*0.0001 ...      % init XYZ accel bias uncertainties, m/s^2
         [1]*0.01 ...      % init time  err 
    ].^2);
...
%     mag2D_yaw_deg           = uavSensors.mag2D_yaw_deg(kTime);
    ...
    ekf.Q = diag([...
            ...
            [1]*ekf.sigmas.timeerr_process_noise ... % XYZ accel bias process noise, m/s^2
        ].^2);
...
%   lag_time=1;
%     lag_time= int16(my_ahrsFeedback.right_delay_time);
    ekf.compute_xdot_and_F = @compute_xdot_and_F__ins_ekf_quaternion_with_time;
    ekf.compute_zhat_and_H = @compute_zhat_and_H__ins_ekf_quaternion_with_time;
    ekf.u1 = gyro_wb_rps_old(lag_time,:);                           % Additional input needed to compute xdot, F, zhat, or H
    ekf.u2 = accel_fb_mps2_old(lag_time,:);                         % Additional input needed to compute xdot, F, zhat, or H
...
%     ekf.u2 = accel_fb_mps2;                         % Additional input needed to compute xdot, F, zhat, or H
    ...
    if GPS_valid
        ...
        ekf.z = [...
               ( mag3D_unitVector_in_body_old(lag_time,:))'; ...               % Magnetometer unit vector [3x1]
...
%         ekf.R = max(ekf.R,(1e-3)^2*eye(size(ekf.R)));    
    ...
    [ekf.xhat ,ekf.P ,ekf.H] = perform_ekf( ...
    ...
    filter.advanced_pos = [ekf.xhat(5); ekf.xhat(6); -ekf.xhat(7) ];
    ...
    my_q = my_q + dt_s*my_qdot;
     ...
     my_q=my_q/norm(my_q);
         ...
         filter.advanced_speed =  filter.advanced_speed+filter.est_acc *dt_s; 
              filter.advanced_pos  =   filter.advanced_pos + filter.advanced_speed *dt_s;
    end
           filter.advanced_pos(3) =  - filter.advanced_pos(3);
    ...
    uavEst.time_err(kTime,:)  =   ekf.xhat(17);     % time_eeror
%    my_data_fuse_ins_feed_back;
%    my_process_method2;
%    my_process_only_create_acc;
