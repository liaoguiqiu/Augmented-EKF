...
uavSensors.sigmas.gyro_noise_rps            = 0.0017;     % Gyro angular rate noise and bias, rad/s
% uavSensors.sigmas.gyro_noise_rps            = 0;

uavSensors.sigmas.gyro_bias_rps             = 0.01;     %
uavSensors.sigmas.accel_noise_mps2          = 0.01;      % Accelerometer noise and bias, m/s^2
% uavSensors.sigmas.accel_noise_mps2          = 0;   
...
% uavSensors.sigmas.GPSpos_noise_m            = 0;        % GPS position noise and bias, m
uavSensors.sigmas.GPSpos_noise_m            = 1;        % GPS position noise and bias, m


uavSensors.sigmas.GPSpos_bias_m             = 0;        %
uavSensors.sigmas.GPSvel_noise_mps          = 0.05;        % GPS velocity noise and bias, m/s
% uavSensors.sigmas.GPSvel_noise_mps          = 0;        % GPS velocity noise and bias, m/s
...
uavSensors.sigmas.mag3D_unitVector_bias     = 0;     %

% Establish GPS update rate
uavSensors.params.dt_GPS_s = 0.1;  % Number of seconds between GPS updates
...
lag_time= 1;
% lag_time= 20;
lengtha = length(uavTruth.east_m);
 
 uavSensors.GPS_east_m = [uavTruth.east_m(1:lag_time) ;uavTruth.east_m(1: lengtha-lag_time)]  ... 
                            + uavSensors.biases.GPS_east_m ...
                            + uavSensors.sigmas.GPSpos_noise_m*randn(size(uavTruth.time_s));
uavSensors.GPS_north_m = [ uavTruth.north_m(1:lag_time) ;uavTruth.north_m(1: lengtha-lag_time)] ...
                            + uavSensors.biases.GPS_north_m ...
                            + uavSensors.sigmas.GPSpos_noise_m*randn(size(uavTruth.time_s));
uavSensors.GPS_h_msl_m =[  uavTruth.h_msl_m(1:lag_time) ;uavTruth.h_msl_m(1: lengtha-lag_time)] ... 
...
for m=[1 2 3] % 1: North, 2: East, 3: Down   
    ...
    uavSensors.GPS_v_ned_mps(:,m) =  [ uavTruth.v_ned_mps(1:lag_time,m) ; uavTruth.v_ned_mps(1: lengtha-lag_time,m)]   ...
...
%??GPS????10HZ
% uavSensors.GPS_valid =ones(length(uavSensors.GPS_valid),1); 
...
for kTime=1:length(uavTruth.time_s)
    ...
    if(kTime>(lag_time+1))
     v_ned_mps      = uavTruth.v_ned_mps(kTime-lag_time,:)';
    end
    ...
    uavSensors.pitot_airspeed_mps(kTime,1) = Vrel_b_mps(1) ...
                            + 1*uavSensors.biases.pitot_airspeed_mps ...
                            + 4* uavSensors.sigmas.pitot_noise_mps*randn;
end
...
% ????????????????????
uavTruth.vdot_ned_mps2 = vdot_ned_mps2;
...
%     fb_mps2_perfect =  - C_ned2b*[0;0;gravity_mps2];
