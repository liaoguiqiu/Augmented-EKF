...
%   2  Premerlani, W., and Bizard, P., 揇irection Cosine Matrix IMU:
...
%     利用反馈消除先加速度
%     gb_ref_mps2 =-(accel_fb_mps2 - my_ahrsFeedback.Acc_err);
    
%     if (my_ahrsFeedback.Acc_err==0)
%         gb_ref_mps2 = cross(gyro_wb_rps,vb_ref_mps) - accel_fb_mps2;  
%     else
%         gb_ref_mps2 =-(accel_fb_mps2 - my_ahrsFeedback.Acc_err);
%     end
%     gb_ref_mps2 =  - accel_fb_mps2;      
  
     
%      gb_ref_mps2 = cross(gyro_wb_rps,vb_ref_mps) - accel_fb_mps2;      
     gb_ref_mps2 = cross(gyro_wb_rps, my_ahrsFeedback.Vb_real) - accel_fb_mps2;
    ...
    %判断有运动加速度
    if( norm(accel_fb_mps2)<7.7|| norm(accel_fb_mps2)>13.0)
%         e_b=e_b*0;
    end
    ...
    gyro_integration_method = 'QUATERNION';  % Options: 'EULER', 'DCM' or 'QUATERNION'
...
%             end
%             v_ned_est_mps_old(1)= -v_ned_est_mps(3);
%        my_ahrsFeedback.Acc_gps=(v_ned_est_mps_old(1)-v_ned_est_mps_old(30))/dt_s/20;
%       my_ahrsFeedback.speed_H_gps  =  -v_ned_est_mps(3);
